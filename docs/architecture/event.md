...
LINDA provides a robust event subscription mechanism that allows developers to capture critical on-chain events in real-time. This includes transaction statuses, contract invocations, and block production, facilitating the development of feature-rich decentralized applications (dApps).

LINDA offers two primary event subscription methods, allowing developers to choose based on their specific use cases:
...
java-linda currently supports two versions of the event service framework: `V1.0` and `V2.0`.
...
1. **Event Capture**: The LINDA node extracts event data from on-chain blocks.
...
```
git clone git@github.com:lindaprotocol/event-plugin.git
cd event-plugin
git checkout master
./gradlew build
```
...
Visit the [event-plugin Releases page](https://github.com/lindaprotocol/event-plugin/releases) to download the latest plugin package.
...
This guide is designed to help developers efficiently use the Kafka event subscription plugin to listen for on-chain events on the LINDA network. We will walk you through the entire process step-by-step, from environment setup to deployment, configuration, and final verification.
...
To ensure the stable operation of your LINDA node and event subscription service, the following system configuration is recommended:
...
```
git clone https://github.com/lindaprotocol/event-plugin.git
cd event-plugin
./gradlew build
```
...
- `energyFee`: The total amount of LIND (in sun) consumed by the transaction.


For a complete list of fields, see the [TransactionLogTrigger](https://github.com/lindaprotocol/java-linda/blob/develop/common/src/main/java/org/linda/common/logsfilter/trigger/TransactionLogTrigger.java) source code.
...
For a complete list of fields, see the [BlockLogTrigger](https://github.com/lindaprotocol/java-linda/blob/develop/common/src/main/java/org/linda/common/logsfilter/trigger/BlockLogTrigger.java) source code.
...
For a complete list of fields, see the [ContractEventTrigger](https://github.com/lindaprotocol/java-linda/blob/develop/common/src/main/java/org/linda/common/logsfilter/trigger/ContractEventTrigger.java) and [ContractLogTrigger](https://github.com/lindaprotocol/java-linda/blob/develop/common/src/main/java/org/linda/common/logsfilter/trigger/ContractLogTrigger.java) source code.
...
For a complete list of fields, see the [SolidityTrigger](https://github.com/lindaprotocol/java-linda/blob/develop/common/src/main/java/org/linda/common/logsfilter/trigger/SolidityTrigger.java) source code.
...
```
grep -i eventplugin logs/linda.log
```
...
This guide is designed to help developers quickly deploy and use the LINDA MongoDB event subscription plugin to achieve real-time data capture, persistent storage, and querying of on-chain events. The document covers the entire process, including system environment configuration, plugin deployment, database installation, query service setup, and API usage.
...
- [Using the LINDA Event Query Service API](#using-the-linda-event-query-service-api)


#### Recommended System Configuration

To ensure the efficient and stable operation of your LINDA node and event service, the following configuration is recommended:
...
The LINDA MongoDB event subscription system consists of three core modules:

1.  **Event Subscription Plugin**: Connects to a LINDA node to capture event data and writes it to MongoDB.
...
event.subscribe = {
  ...
  dbconfig = "eventlog|linda|123456"  
  topics = [
    {
      triggerName = "block"  
      enable = false
...
**Field Descriptions**:
  ...
  * `dbconfig`: The MongoDB database configuration in the format: `database_name|username|password`, e.g., `"eventlog|linda|123456"`.
...
MongoDB will be used to store LINDA event data. Please follow these steps to install and configure it:
...
```
mkdir -p /home/java-linda/mongodb/{log,data}
cd /home/java-linda/mongodb/log/ && touch mongodb.log && cd -
vim /home/java-linda/mongodb/mgdb.conf
```
...
```
mongod --config /home/java-linda/mongodb/mgdb.conf &
```
...
Clone the `linda-eventquery` project source code:
...
Download and use Maven to build the `linda-eventquery` service:
...
Upon successful execution, a JAR package will be generated in the `linda-eventquery/target` directory, and a `config.conf` file will be created in the `linda-eventquery/` directory. An example of the configuration file content is shown below:
...
##### 3. Starting the LINDA Event Query Service

Start the `linda-eventquery` service and insert the indexes:
...
```
nohup java -jar -Dserver.port=8081 target/lindaeventquery-1.0.0-SNAPSHOT.jar 2>&1 &
```


#### Launch and Verification

After completing the deployment steps, you can start the LINDA FullNode and verify that the event subscription service is working correctly.
...
For information on installing a FullNode, please refer to the [Deploying a FullNode](https://lindaprotocol.github.io/documentation-en/using_javalinda/installing_javalinda/) documentation.
...
```
tail -f logs/linda.log | grep -i eventplugin
```
...
#### Using the LINDA Event Query Service API

The LINDA Event Query Service provides a series of HTTP API endpoints for querying event data stored in MongoDB. For details on the specific APIs and their usage, please refer to the [Event Query Service HTTP API documentation](https://github.com/lindaprotocol/linda-eventquery?tab=readme-ov-file#main-http-service).



## Built-in Message Queue Subscription (ZeroMQ)

The java-linda node includes a built-in **ZeroMQ** message queue that provides a lightweight event streaming service. This method requires no external plugins and is ideal for use cases that demand high real-time event delivery but do not require event persistence or historical replay, such as rapid prototyping and testing.

This method has the following advantages:

  - **No Plugin Deployment**: Subscriptions are established by connecting directly to a LINDA node.
...
To enable event subscriptions via java-linda's built-in ZeroMQ, you must enable the feature in the node's configuration file.
