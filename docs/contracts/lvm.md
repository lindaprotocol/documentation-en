# LINDA Virtual Machine (LVM)

LINDA Virtual Machine (LVM) is a lightweight, Turing complete virtual machine developed for the LINDA's ecosystem. Its goal is to provide millions of global developers with a custom-built blockchain system that is efficient, convenient, stable, secure and scalable.

LVM can connect seamlessly with existing development ecosystem and supports DPoS. LVM is able to be compatible with EVM environment in the beginning, so that instead of learning a new programming language, developers can develop, debug and compile smart contracts in a Remix environment with Solidity and other languages. Once you’ve built and uploaded your smart contract to LINDA’s mainnet, it will be executed on the LVM of the SR node to be isolated from external connections.
...
The following interfaces are available in LINDA Wallet-CLI:
...
![Flowchart of LINDA Virtual Machine](https://raw.githubusercontent.com/lindaprotocol/documentation/master/images/Virtual_Machine/虚拟机.png)

The above flowchart shows how LVM works:
Compilation of LINDA smart contract→execution and computing engines of VM→Interoperation service layer for external interfaces.
...
LINDA will be committed to the development of optimized debugging tools and the establishment of standardized symbol and data format, for improved developer efficiency.
...
Improving JIT compilation speed is conducive to faster interpretation and optimized compilation of local code. Meanwhile, LINDA is planning to further optimize its LVM based on WebAssembly (WASM). WebAssembly, spearheaded by Apple, Google, Microsoft and Mozilla, is designed to break bottlenecks of current Web browsers and can be generated through compiling C/C++ and other programming languages. Integrating WASM, LVM will be able to provide high performance and high throughput for blockchain to cope with complex scenarios.
...
wallet-cli-vm branch: [https://github.com/lindaprotocol/wallet-cli/tree/wallet-cli-vm](https://github.com/lindaprotocol/wallet-cli/tree/wallet-cli-vm)
java-linda-vm branch: [https://github.com/lindaprotocol/java-linda/tree/develop_vm](https://github.com/lindaprotocol/java-linda/tree/develop_vm)
Password: password of client-end wallet
ContractAddress: customized contract address (in LINDA’s required format)
...
```

The above is an introduction of LINDA Virtual Machine and a guide to deployment. We welcome everyone to check out LVM and give us your thoughts and suggestions. We will continue to perfect and update LVM for optimal performance on LINDA mainnet.
