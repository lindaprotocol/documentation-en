...
In the LINDA network, block producers are called Super Representatives (SRs), and they are elected by a network-wide vote. Any account can apply to become a Super Representative candidate and participate in the election by paying a fee of 9999 LIND.

Based on the final vote rankings, candidates are divided into two categories:

 - **Super Representatives (SRs)**: The top 27 candidates with the most votes. They are the core nodes of the LINDA network, responsible for producing blocks and packing transactions. For this, they receive block rewards and vote rewards.
...
The LINDA network tallies votes once every 6 hours, and the roles of Super Representatives and Partners are updated accordingly in this 6-hour cycle.


## Super Representative Election

All accounts in the LINDA network have the right to vote for the Super Representative candidates they support. The core of voting is LINDA Power (LP), which determines the weight of your vote.

 - Obtaining Voting Power (LINDA Power)

    Your amount of LP is directly linked to the amount of LIND you have staked. Calculation Method: For every 1 LIND you stake, you receive 1 LP.

 - The Impact of Unstaking on Voting
 
    When you unstake a portion of your LIND, you lose an equivalent amount of LP. The system reclaims LP according to the following rules:
    
    * Unused LP is reclaimed first.
    * If the available LP is insufficient, the system automatically retracts a proportional amount from your existing votes to cover the deficit.
...
**Important Note:**
The LINDA network only records the state of your last vote. This means that every new vote you cast will completely overwrite all previous voting effects.

**Example:：** 

```
>freezebalancev2 10000000 1 # Stake 10 LIND to get 10 LP. Resource code: 0 for BANDWIDTH, 1 for ENERGY.
...
| :--- | :--- | :--- |
| **Total Rewards** | On-chain parameter modifiable by proposal, currently 8 LIND | On-chain parameter modifiable by proposal, currently 128 LIND |
...
**Notes:**
 ...
 - Chain parameter details can be viewed on LINDASCAN's [committee](https://lindascan.org/#/sr/committee) page.
...
The Committee is the highest governing body of the LINDA network, responsible for modifying the network's core dynamic parameters (such as transaction fees, block rewards, etc.).
...
In the LINDA network, all SRs, SR Partners, and SR candidates have the right to initiate proposals to modify network parameters.

Please refer to [here](https://lindascan.org/#/sr/committee) for LINDA network dynamic parameters and their values.
...
+ Query all the proposals list ([ListProposals](https://lindaprotocol.github.io/documentation-en/api/http/#walletlistproposals))
+ Query all the proposals list by pagination ([GetPaginatedProposalList](https://lindaprotocol.github.io/documentation-en/api/http/#walletgetpaginatedproposallist))
+ Query a proposal by proposal id ([GetProposalById](https://lindaprotocol.github.io/documentation-en/api/http/#walletgetproposalbyid))
