# LINDA DPoS Consensus Mechanism
...
Currently, mainstream consensus mechanisms include Proof of Work (PoW), Proof of Stake (PoS), and Delegated Proof of Stake (DPoS), and each mechanism has its unique specific implementation in different blockchain systems. This document will focus on the DPoS consensus mechanism adopted by the **LINDA network** and elaborate on its core components and operational principles.
...
1. **Initial Setup:** When the blockchain system launches, a certain amount of **tokens** (e.g., LINDA's LIND) are issued.
...
The following will provide a detailed explanation, integrating the specific implementation within the LINDA network.

## Core Concept Definitions

| Concept | Definition |
| -------- | -------- |
| LINDA     | The LINDA blockchain network. This document does not distinguish between LINDA, LINDA blockchain, LINDA blockchain system, etc.|
|LIND| The equity token issued and circulated within the LINDA blockchain system, with the ticker symbol **LIND**.|
|Super Representative (SR) |A node in the LINDA network that has obtained block production qualifications. LINDA sets the number of SRs at **27**. This document does not distinguish between block producer, Witness, SuperNode, SR, etc.|
|Super Representative Candidate| A node in the LINDA network that is eligible to become an SR.|
...
|Block Production / Producing Blocks|The process of validating transactions and recording them as entries. Since entries in LINDA are carried by blocks, the block production process is also referred to as "producing blocks." This document does not distinguish between block production and producing blocks.|
...
|Slot|In the LINDA network, every **3 seconds** is counted as a slot. Under normal circumstances, each SR producing a block will complete block generation within its corresponding slot time. Therefore, LINDA's average block generation interval is approximately 3 seconds. If an SR fails to produce a block for some reason, the corresponding slot will be empty, and the next SR will produce a block in the subsequent slot. During maintenance periods, block production skips 2 slots.|
|Epoch|LINDA defines every **6 hours** as an epoch. The last two block generation times in each epoch are **maintenance periods**. The maintenance period of each epoch will determine the block generation order for the next epoch.|
|Maintenance Period|LINDA sets this at **2 block times**, which is **6 seconds**. This period is used to tally the votes for candidates. Since there are 4 epochs in 24 hours, there are naturally 4 maintenance periods. No blocks are produced during the maintenance period; its primary purpose is to determine the block production order for the next epoch.|

![Epoch](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/sequence_en.jpg)

## Block Production Process

SRs in the LINDA blockchain system continuously execute the following process:
...
SRs continuously repeat this process to ensure that all new transaction data in the LINDA system is promptly recorded onto the ledger.

## Super Representative Election Mechanism

### Votes

Before voting for SRs, an account needs to acquire **voting power**, specifically **LINDA Power (LP)**. Voting power is obtained by **staking LIND**. In addition to obtaining Bandwidth or Energy, staking LIND simultaneously grants users voting power. For every **1 LIND** staked, a user receives **1 LP**.

When an account unstakes LIND, it loses the corresponding amount of LP, and its current votes become invalid. Votes are tallied every **6 hours**, and SRs and SRPs are also updated every 6 hours. If an account casts multiple votes before the vote tally, the LINDA network will only record the account's **latest vote**, and previous votes will be overwritten.

### Voting Process

In LINDA, voting for candidates is defined as a special type of **transaction**. Nodes can vote for candidates by generating a voting transaction.
...
![blockchain_structure](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/blockchain_structure.png)
...
![longest_chain1](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/longest_chain1_en.jpg)
![longest_chain2](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/longest_chain2_en.jpg)

As mentioned earlier, the normal operation of a blockchain system is predicated on the vast majority of nodes in the system being honest and reliable. To further delve into this issue, the primary guarantee for blockchain system security is ledger security: the ledger cannot be maliciously written with illegal data, and the copies of the ledger stored on various nodes should remain consistent. From the perspective of DPoS consensus, the block production process is completed by SRs, so LINDA's security depends on the reliability of the majority of SRs.

LINDA has established an **irreversible block** (or **solidified block**) mechanism, and to resist malicious behavior from a few block-producing nodes, LINDA adopts the **longest chain principle** to confirm the main chain.
...
To ensure the secure and efficient operation of the LINDA blockchain system, LINDA has designed an incentive model to encourage more nodes to join the LINDA network, thereby expanding network scale.

* **Block Production Rewards:** For every block successfully produced in the LINDA network, the corresponding SR is rewarded with **8 LIND**. After deducting their commission based on a self-defined commission rate, this SR distributes the remaining portion to the voters who supported them, based on their voting weight.
* **Voting Rewards:** With the generation of each block, the LINDA network also awards an additional **128 LIND** to all SRs and SRPs. This reward is distributed based on their respective vote proportions. Similarly, after deducting their commission based on their set commission rate, SRs and SRPs distribute the remaining voting rewards to their respective voters based on the voters' voting weight.
...
For current dynamic parameters and their values in the LINDA network, as well as past proposal records, please refer to the [LINDASCAN Committee](https://lindascan.org/#/sr/committee) Page.
