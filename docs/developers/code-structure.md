# java-linda Core Modules
## Code Structure
java-linda is a LINDA network client developed based on the Java language. It implements all the functions mentioned in the LINDA white paper, including consensus mechanism, cryptography, database, LVM virtual machine, network management, etc. We can run a LINDA node by starting java-linda. In this article, we will describe the code structure of java-linda in detail, and introduce the functions of its various modules, to facilitate the subsequent code analysis and development of developers.

java-linda adopts a modular code structure; the code structure is clear and easy to maintain and expand. Currently java-linda is divided into 7 modules: [protocol](#protocol), [common](#common), [chainbase](#chainbase), [consensus](#consensus), [actuator](#actuator), [crypto](#crypto), [framework](#framework), the following introduces the functions of each module and its code organization.
...
The above protocols adopt the [`Google Protobuf`](https://developers.google.com/protocol-buffers) data exchange format. Compared with JSON and XML, the `Google Protobuf` format is more efficient and flexible and can be compiled by the ProtoBuf compiler to generate language-specific serialization and deserialization source code for the defined protocol files. Protobuf is the basis for java-linda to achieve cross-language and cross-platform.

[protocol](https://github.com/lindaprotocol/java-linda/tree/develop/protocol) module's source code is located at:  `https://github.com/lindaprotocol/java-linda/tree/develop/protocol` , its directory structure is as follows:
...
|-- protos
    ...
    |-- core
        |-- Discover.proto
        |-- Linda.proto
        |-- LindaInventoryItems.proto
        |-- contract
```

* `protos/api/` - The gRPC interface and data structure provided by the java-linda node externally
* `protos/core/` - Data structure for communication between nodes and between modules within nodes
    ...
    * `LindaInventoryItems.proto` - Data structure related to block transferring between nodes
    * `contract/` - Contract related data structures
    * `Linda.proto` - Other important data structures, including accounts, blocks, transactions, resources, super representatives, voting, and proposals...
...
[common](https://github.com/lindaprotocol/java-linda/tree/develop/common) module's source code is located at:  `https://github.com/lindaprotocol/java-linda/tree/develop/common`, its directory structure is as follows:

```
|-- /common/src/main/java/org/linda
    |-- common
    |   |-- args
    |   |-- config
    |   |-- entity
...
[chainbase](https://github.com/lindaprotocol/java-linda/tree/develop/chainbase) module's source code is located at: `https://github.com/lindaprotocol/java-linda/tree/develop/chainbase`, its directory structure is as follows:
```
|-- chainbase.src.main.java.org.linda
    |-- common
    |   |-- bloom
    |   |-- error
    |   |-- overlay
...
* `core/` - The core code of the chainbase module
    ...
    * `store/` 
        ...
        Various databases, such as `AccountStore`, `ProposalStore`, etc. `AccountStore` is the account database, the database name is `account`, which stores all account information in the LINDA network; `ProposalStore` is the proposal database, and the database name is `proposal`, which stores all the proposal information in the LINDA network.

    * `db/` and `db2/` 
    
        Implemented rollbackable databases, including two rollbackable databases: `AbstractRevokingStore` located in the `db/` directory and `SnapshotManager` located in the `db2/` directory. Compared with `AbstractRevokingStore`, `SnapshotManager` has a more stable data rollback function and supports the extension of the underlying database. Therefore, java-linda uses `SnapshotManager` to roll back the database. Several important interfaces and implementation classes are as follows:
        ...
        * `LindaStoreWithRevoking.java` - It is the base class that supports rollbackable databases. All rollbackable databases are their implementations, such as `BlockStore`, `TransactionStore`, etc
...
[consensus](https://github.com/lindaprotocol/java-linda/tree/develop/consensus) module's source code is located at:  `https://github.com/lindaprotocol/java-linda/tree/develop/consensus`, its directory structure is as follows:
```
|-- consensus/src/main/java/org/linda/consensus
    |-- Consensus.java
...
[actuator](https://github.com/lindaprotocol/java-linda/tree/develop/actuator) module's source code is located at: `https://github.com/lindaprotocol/java-linda/tree/develop/actuator`, its directory structure is as follows:
```
|-- actuator/src/main/java/org/linda/core
    |-- actuator
...
* `actuator/` - The executors of various types of transactions in the LINDA network which define the processing logic of different types of transactions. For example, `TransferActuator` is the processing class for transferring LIND, and `FreezeBalanceV2Actuator` is the processing class for staking LIND to obtain resource
* `utils/` - tools needed to execute transaction
* `vm/` - LINDA virtual machine related code
...
### crypto
Crypto is a relatively independent module, but it is also a very important module. Data security in java-linda is almost entirely guaranteed by this module. Currently, SM2 and ECKey encryption algorithms are supported.

[crypto](https://github.com/lindaprotocol/java-linda/tree/develop/crypto) module's source code is located at: `https://github.com/lindaprotocol/java-linda/tree/develop/crypto`, its directory structure is as follows:
```
|-- crypto/src/main/java/org/linda/common/crypto
...
The framework is the core module of java-linda and the entrance of the node. The framework module is responsible for the initialization of each module and business logic. The framework module includes the services provided externally, the node discovery and node management process related to the P2P network, and the block broadcasting and processing procedures.

[framework](https://github.com/lindaprotocol/java-linda/tree/develop/framework) module's source code is located at:  `https://github.com/lindaprotocol/java-linda/tree/develop/framework`, its directory structure is as follows:

```
|-- framework/src/main/java/org/linda
    |-- common
    |   |-- application
    |   |-- backup
    |   |-- logsfilter
...
The database module of java-linda is also called the ChainBase module. This article mainly introduces some background knowledge and shows developers the implementation details of the ChainBase module by introducing logic such as transaction processing, state rollback, and data persistence.



### Prerequisites
The database is an important part of the blockchain system. It stores all the data on the blockchain and is the basis for the normal operation of the blockchain system. Each fullnode stores a full amount of data, including block data, state data, etc. java-linda uses the Account model to save the user's account state.
...
To solve the problems faced by PoW, some people proposed PoS (Proof of Stake), which is simply understood as the more coins that the node holds, the greater the probability of obtaining the right to produce blocks, but this will lead to monopoly problems as well. In order to improve, DPoS (Delegated Proof of Stake) is proposed: the decentralization feature is guaranteed by the elected super representative, and the super representative is responsible for the block production in turn to improve the efficiency. java-linda currently adopts the DPoS consensus mechanism.
...
Based on this consideration, java-linda uses LevelDB as the underlying data storage by default, and java-linda has a good architecture design. The interface-oriented programming mode makes the chainbase module have better scalability. All databases implemented the chainbase interface can be used as the underlying storage engine of java-linda. For example, in the chainbase v2 version, a database implementation based on RocksDB is provided.


#### Transaction Validation
As we all know, the blockchain mainly stores transaction data. Before introducing the chainbase module, you need to understand the transaction processing logic in java-linda.

![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_1.png)
...
**Note: The specific implementation of java-linda deviates from the above figure, and for the sake of convenience, this article collectively refers to the FullNode and SR as the nodes.**

For example, to process a transfer transaction: user A transfers 100 LIND to user B, and it needs to validate whether user A has enough balance to make the transfer.

The account library in the database stores the account information of all users, including the user's balance information. How to judge whether this transfer transaction is legal? The logic of java-linda is: when a transaction is received from the network, the transaction operation will be executed immediately, that is, the account information will be modified in the local database: (accountA - 100LIND, accountB + 100LIND). If this operation can be executed successfully, it means that the transaction is legal at least in the current state, and can be packed into the block.
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_2.png)




If the account balance of accountA is 100 at the block height is 1000, the node receives and validates a transaction 't1', in which accountA transfers 100LIND to accountB. After receiving the new block1001, the block contains a transaction 't2', in which accountA transfers 50LIND to accountC. In theory, t2 has been packed into the block, and the priority is higher than t1. However, if no operation is done, the validation of t2 will fail because accountA does not have enough balance. Therefore, after receiving the new block 1001, the state change generated by transaction t1 needs to be rolled back.
...
Because java-linda does a secondary validation of the transaction when it is packed into the block. The secondary validation is due to the timeliness of the transaction. Still taking the above figure as an example, it can be seen from the figure, that after 1001 is received, the transaction t1 was rolled back, and the balance of accountA was deducted by 50. And then, it was the node's turn to produce a block, but t1 had become an illegal transaction at this time because the balance in accountA was not enough to transfer 100 LIND, it is not advisable to directly pack t1 into the block. So the transaction needs to be validated again, which is why the transaction needs to be validated twice when producing a block.
...
java-linda maintains a data structure in memory as below,

![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_3.png)



java-linda holds all blocks that have not reached consensus recently. When a forked chain occurs, according to the longest chain principle: if the block height of the forked chain is greater than the current main chain block height, the forked chain needs to be switched to the main chain. Part of the blocks on the previous main chain needs to roll back up to their common parent blocks when switching, and then apply new main chain blocks sequentially from the parent block.
...
This chapter explains receiving and validating transactions, block production, validating and saving blocks from the perspective of code, to further analyze the chainbase module of java-linda. If there is no further declaration, the default description is dedicated to all the Fullnode (including SR).


#### Receiving Transactions
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_4.png)


After the node receives a transaction, it puts the transaction into the local pushTransactionQueue cache queue by calling the `pushTransaction(final TransactionCapsule lind)` function of the manager class and validates the transaction at the same time. And the return of this method is sort of elegant:
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_5.png)


When rolling back, java-linda move the transactions in the pendingTransactionQueue to rePushTransactions, and clear the pendingTransactionQueue, see the figure for a detailed explanation.
...
There is a session object in java-linda. A session represents the change in the state of a block. The session object is mainly used for rollback. For example,rolling back the state to the state of the previous block needs to be operated throughout the session, as shown in the following figure,

![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_6.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_7.png)
...
The database storage of java-linda is slightly different from other public chains. For example, the Ethereum persistence layer uses only one database instance, and different types of data in Ethereum are distinguished by prefixes and stored in one database instance. However, java-linda currently stores data of different business types in its own database instances.
...
In order to ensure the atomic writing of multiple database instances, java-linda has added a checkpoint mechanism, which writes the changed data to the checkpoint uniformly before the multiple instances are placed on the disk. If an accident occurs in writing to multiple database instances, the changed data will be recovered from the checkpoint when the service is restarted to ensure the atomicity of writing.
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/chainbase_9.png)
...
#### LINDA Network
The architecture diagram of LINDA is as follows:
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_architecture.png)

As the most fundamental module of LINDA, the P2P network directly determines the stability of the entire blockchain network. The network module can be divided into the following four parts according to the function:
...
### Node Discovery
Node discovery is the first step for nodes to access the blockchain network. The blockchain network is a structured P2P network which organizes all nodes in an orderly manner, such as forming a ring network or a tree-like network. Structured networks are generally implemented based on the DHT (Distributed Hash Table) algorithm. Specific implementation algorithms include Chord, Pastry, CAN, Kademlia and so on. The LINDA network uses the Kademlia algorithm.
...
#### Kademlia Implementation by LINDA
The main points of the Kademlia algorithm implemented by LINDA are as follows:
...
* K-Bucket: The node routing table. According to the distance between the nodes, the remote nodes are divided into different buckets. The remote nodes with the same distance as the current node are recorded in the same bucket, and each bucket can accommodate up to 16 nodes. According to the calculation formula of node distance, it can be seen that the Kademlia algorithm implemented by LINDA maintains a total of 256 buckets.


The node discovery protocol of LINDA includes the following four UDP messages:
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_discoverinit.png)
...
* `DiscoverTask` is to discover more nodes that are closer to myself. It is executed every 30s. The execution flow is as follows:
    ![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_discovertask.png)
* `RefreshTask` is to expand the local k-bucket by random node ID, that is, to find nodes that are closer to the random node ID. It is executed every 7.2s. The execution process is as follows:
    ![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_refreshtask.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_updatek.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_connect.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_filterrule.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_syncflow.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_sync1.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_sync2.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_sync3.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/network_broadcastflow.png)
...
### Summary
This article introduces the implementation details related to the P2P network, the lowest level module of LINDA, including node discovery, node connection, block synchronization, and the process of block and transaction broadcasting. I hope that reading this article can help developers to further understand and develop java-linda network-related modules.
