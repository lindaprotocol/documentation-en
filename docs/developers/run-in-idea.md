#  IntelliJ IDEA Configuration for java-linda

To simplify the Java development process and improve efficiency, selecting and configuring an Integrated Development Environment (IDE) is a crucial first step. This guide uses IntelliJ IDEA as an example to explain how to set up and configure the java-linda development environment.

java-linda nodes support deployment on `Linux` or `MacOS` operating systems. The JDK version requirements are as follows:
...
Lombok simplifies Java code through annotations and is an essential plugin for java-linda development.
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_annotation.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_JDK.png)
...
Clone the java-linda source code to your local machine and switch to the `develop` branch.
...
### Step 2: Compile java-linda

You can compile the project in two ways:

  * **Compile using the terminal:**
 
    In the root directory of the `java-linda` project, execute the following Gradle command:
  ...
  * **Compile using the IntelliJ IDEA UI:**
    ...
    Open the `java-linda` project in IntelliJ IDEA, then click `Build` -\> `Build Project` in the top menu bar to compile the entire project.


## Configuring Code Style Checks

java-linda adheres to the `Google checkstyle` code standard. By configuring the `Checkstyle` plugin in IDEA, you can check your code style in real-time to ensure the quality of your commits.
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_checkstyle.png)

### Step 2: Configure Checkstyle Rules

1. First, locate the code style configuration file. You can use the local `config/checkstyle/checkStyleAll.xml` file within the project, or download it from the official [GitHub Repository](https://github.com/lindaprotocol/java-linda/blob/develop/config/checkstyle/checkStyleAll.xml).
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_checkStyleAll.png)
4. In the dialog that appears, set the  `Description` to `linda-checkstyle` and select the `checkStyleAll.xml` file you just obtained.
5. Select the checkbox for the newly added `linda-checkstyle` rule and click "Apply" and "OK".
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_stylecheck.png)
...
Before running java-linda, you need to create a working directory to store the database and log files generated by the node at runtime.

```
mkdir /Users/javalindadeploy
```

> **Important Note**: java-linda will look for the `config.conf` file in this directory. Before starting the node, ensure you have placed the correct configuration file in this directory.


### Step 2: Configure Run/Debug Options

Next, create a new run configuration in IntelliJ IDEA to define how it launches the java-linda application.
...
3. Find and set the following options in order:
      ...
      * **Main Class:** Set to `org.linda.program.FullNode`.
      * **Program Arguments:** Enter the node startup arguments. For example, use `-c config.conf` to specify the configuration file.
      * **Working Directory:** Set this to the directory you created in [Step 1](#rndstep1), e.g., `/Users/javalindadeploy`。
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-zh/master/images/IDE_RunDebug.png)
4. Click “Apply” to save the configuration.

### Step 3: Start the Node

Now you can start the java-linda node from within IDEA:

  * **Run the node:** Click `Run` -\> `Run 'FullNode'` in the top menu bar.
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-zh/master/images/IDE_runjavalinda.png)
...
![image](https://raw.githubusercontent.com/lindaprotocol/documentation-en/master/images/IDE_debug.png)
